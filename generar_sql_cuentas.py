        ...
        for sheet_name in xl.sheet_names:
            ...
            if sheet_name == xl.sheet_names[0]:
                ...
                for i, col in enumerate(df.columns):
                    col_name = col.lower().replace(' ', '_').replace('(', '').replace(')', '').replace('-', '_').replace('ó', 'o')
                    if i == 0:  # Primera columna (nivel)
                        print(f"  {col_name} VARCHAR(10),")
                    elif i == 1:  # Segunda columna (código)
                        print(f"  {col_name} VARCHAR(50) NOT NULL,")
                    elif i == 2:  # Tercera columna (nombre)
            ...
            for col in df.columns:
                col_name = col.lower().replace(' ', '_').replace('(', '').replace(')', '').replace('-', '_').replace('ó', 'o')
                columnas_excel.append(col_name)
